 ...
 */
public class DatacenterBrokerTime extends SimEntity {
	
	protected List waitCloudlet;
	/** The vm list. */
	 ...
	 * @post $none
	 */
	public DatacenterBrokerTime(String name) throws Exception {
		super(name);
		waitCloudlet= new ArrayList();
	...
	/*protected void processCloudletReturn(SimEvent ev) {
		...
		{
			boolean exist=false;
			for(Cloudlet c:getCloudletReceivedList())
			{
			...
			if(!exist)
			{
				...
				waitCloudlet.add(cloudlet);
				return;
			}
		}else if(cloudlet.getCloudletId()==1 && waitCloudlet.size()>0)
			{
				Cloudlet w = (Cloudlet)waitCloudlet.get(0);
				processCloudletReturn(cloudlet);
				processCloudletReturn(w);				
		}else
			processCloudletReturn(cloudlet);
	}*/
